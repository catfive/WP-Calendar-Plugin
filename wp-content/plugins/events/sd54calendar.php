...
function UpcomingEvents() {
	parent::WP_Widget(false, $name = 'Upcoming Events');	
}
...
function widget($args, $instance) {		
	...
	$widgetq = new WP_query($widgetqargs);
	global $post;
...
?>	
	...
	<ul>
		<?php while ( $widgetq->have_posts() ): $widgetq->the_post();
			$EventStartTime = get_post_meta($post->ID, 'event_date_start', true);
			$EventStartDayInt = date('j', $EventStartTime);?>
			<li><a href="<?php the_permalink()?>"><?php echo date('m/d', $EventStartTime) . ": "; the_title();?></a></li>
		<?php endwhile;?>
	</ul>
